...
const updateAccountDetails=asyncHandler(async(req,res)=>{
    ...
    const user=await User.findByIdAndUpdate(
        ...
        {new:true}//update hone k baad jo information hoti hai vo idhar return hoti hai 
    ).select("-password")
...
const updateUserCoverImage=(async(req,res)=>{
    ...
    .status(200)
    .json(
        200,user,"CoverImage is uploaded successfully"
    )
})
...
const getWatchHistory=asyncHandler(async(req,res)=>{
    ...
    .json(
        new ApiResponse(
            200,
            user[0].watchHistory,
            "watchHistory is created successfully"
        )
    )
})
export { registerUser,
